...
export default function OnboardingWizard() {
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 flex items-center justify-center bg-[#0B0F19] px-4 sm:px-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl glass rounded-2xl p-6 sm:p-8 shadow-2xl relative z-10 animate-fade-in border border-[#23324C]/80">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#23324C]/60 pb-6 mb-8 gap-4">
          <div>
            <span className="font-extrabold text-lg tracking-tight text-white flex items-center gap-2">
              <img src={heroLogo} alt="Hero Logistics Logo" className="h-7 w-auto object-contain hover:scale-105 transition-transform duration-300" />
            </span>
        ...
        <div className="hidden md:flex justify-between items-center mb-10 px-4 relative">
          {/* Connector Line */}
          <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-[#23324C] -translate-y-1/2 z-0"></div>
          <div 
        ...
        <div className="min-h-[380px] bg-[#111827]/40 border border-[#23324C]/60 rounded-xl p-5 sm:p-7 mb-6 animate-fade-in-up">
          ...
          {currentStep === 1 && (
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6">Setup the legal entity name, taxation codes, and system localization preferences.</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Company Legal Name</label>
                    <input
                      type="text"
                      ...
                      className="block w-full px-4 py-3 bg-[#161f30]/60 border border-[#23324C] focus:border-brand-500 rounded-xl text-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Tax Registration / VAT ID</label>
                    <input
                      type="text"
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 uppercase mb-1.5">System Currency</label>
                      <select 
                        value={companyDetails.currency}
                        onChange={(e) => setCompanyDetails({...companyDetails, currency: e.target.value})}
                        className="block w-full px-4 py-3 bg-[#161f30] border border-[#23324C] focus:border-brand-500 rounded-xl text-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 cursor-pointer"
                      >
                        <option>USD ($)</option>
                        <option>EUR (€)</option>
                        <option>INR (₹)</option>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Timezone</label>
                      <select 
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Brand Logo Asset</label>
                    <div className="border-2 border-dashed border-[#23324C] hover:border-brand-500/50 rounded-xl p-4 text-center cursor-pointer transition-colors bg-[#161f30]/20 flex flex-col items-center justify-center">
                      <Upload className="h-6 w-6 text-slate-400 mb-2" />
                      <span className="text-xs text-slate-400 font-medium">Drag logo file here or <span className="text-brand-400">browse</span></span>
          ...
          {currentStep === 2 && (
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6">Create physical office or warehouse locations. Add at least one warehouse terminal or office.</p>

              {/* Add form */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 bg-[#161f30]/40 p-4 border border-[#23324C] rounded-xl mb-6">
                <input
                  type="text"
                  placeholder="Branch Name (e.g. Chicago Depot)"
                  value={newBranch.name}
                  onChange={(e) => setNewBranch({...newBranch, name: e.target.value})}
                  className="px-3.5 py-2 bg-[#111827] border border-[#23324C] focus:border-brand-500 rounded-lg text-slate-200 text-xs focus:outline-none"
                />
                <input
                  type="text"
                  placeholder="City"
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="w-full px-3.5 py-2 bg-[#111827] border border-[#23324C] focus:border-brand-500 rounded-lg text-slate-200 text-xs focus:outline-none"
                  />
                  <button
                    onClick={addBranch}
              ...
              <div className="space-y-2.5">
                <label className="block text-xs font-bold text-slate-400 uppercase">Configured Branches ({branches.length})</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 max-h-48 overflow-y-auto pr-1">
                  {branches.map((b) => (
                    <div key={b.id} className="flex items-center justify-between p-3.5 bg-[#161F30] border border-[#23324C] rounded-xl hover:border-brand-500/30 transition-all">
                      <div>
                        <h4 className="text-sm font-extrabold text-slate-200">{b.name}</h4>
                        <span className="text-xs text-slate-400">{b.city}, {b.state}</span>
                      </div>
                      {branches.length > 1 && (
          ...
          {currentStep === 3 && (
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6">Invite dispatchers, accounts executives, yard managers, and drivers to log in.</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 bg-[#161f30]/40 p-4 border border-[#23324C] rounded-xl mb-6">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="w-full px-3.5 py-2 bg-[#111827] border border-[#23324C] focus:border-brand-500 rounded-lg text-slate-200 text-xs focus:outline-none cursor-pointer"
                  >
              ...
              <div className="overflow-x-auto max-h-48 overflow-y-auto">
                <table className="min-w-full text-left text-xs border border-[#23324C] rounded-xl">
                  <thead className="bg-[#161F30] text-slate-300 uppercase font-semibold">
                    <tr>
                  ...
                  <tbody className="divide-y divide-[#23324C]/60 text-slate-300">
                    {users.map((u) => (
          ...
          {currentStep === 4 && (
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6">Configure active vehicles in your pool to enable routing and dispatch tracking cards.</p>

              {/* Add form */}
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 bg-[#161f30]/40 p-4 border border-[#23324C] rounded-xl mb-6">
                <input
                  type="text"
                ...
                <select
                  ...
                  className="px-3.5 py-2 bg-[#111827] border border-[#23324C] focus:border-brand-500 rounded-lg text-slate-200 text-xs focus:outline-none cursor-pointer"
                >
              ...
              {/* Grid List */}
              <div className="space-y-2.5">
                <label className="block text-xs font-bold text-slate-400 uppercase">Vehicles In Fleet ({vehicles.length})</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 max-h-48 overflow-y-auto pr-1">
                  {vehicles.map((v) => (
                    <div key={v.id} className="flex items-center justify-between p-3.5 bg-[#161F30] border border-[#23324C] rounded-xl hover:border-brand-500/30 transition-all">
                      <div>
                        <h4 className="text-sm font-extrabold text-slate-200 flex items-center gap-2">
                          <Truck className="h-4 w-4 text-brand-400" />
                          {v.plate}
                        </h4>
                        <span className="text-xs text-slate-400">{v.type} • {v.capacity} • {v.branch}</span>
                      </div>
                      {vehicles.length > 1 && (
          ...
          {currentStep === 5 && (
            <div>
              ...
              <p className="text-slate-400 text-xs mb-6">Define shipper accounts or partners to assign loads and invoices to immediately.</p>
              ...
              {/* Grid List */}
              <div className="space-y-2.5">
                <label className="block text-xs font-bold text-slate-400 uppercase">Shippers Configured ({customers.length})</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 max-h-48 overflow-y-auto pr-1">
                  {customers.map((c) => (
                    <div key={c.id} className="flex items-center justify-between p-3.5 bg-[#161F30] border border-[#23324C] rounded-xl hover:border-brand-500/30 transition-all">
                      <div>
                        <h4 className="text-sm font-extrabold text-slate-200">{c.name}</h4>
                        <span className="text-xs text-slate-400">{c.contact} • {c.email} • {c.terms}</span>
                      </div>
                      {customers.length > 1 && (
          ...
          {currentStep === 6 && (
            <div className="text-center py-6">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Onboarding Configuration Complete!</h3>
              <p className="text-slate-400 text-sm max-w-md mx-auto mb-8">
                Your company workspace is fully provisioned. The items configured below are ready to be used.
              </p>

              {/* Summary Cards */}
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5 max-w-3xl mx-auto mb-8">
                <div className="p-3 bg-[#161F30] border border-[#23324C] rounded-xl">
                  <span className="block text-xl font-black text-brand-400">{companyDetails.legalName ? '1' : '0'}</span>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Company Profile</span>
                </div>
                <div className="p-3 bg-[#161F30] border border-[#23324C] rounded-xl">
                  <span className="block text-xl font-black text-brand-400">{branches.length}</span>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Branches</span>
                </div>
                <div className="p-3 bg-[#161F30] border border-[#23324C] rounded-xl">
                  <span className="block text-xl font-black text-brand-400">{users.length}</span>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Users Invited</span>
                </div>
                <div className="p-3 bg-[#161F30] border border-[#23324C] rounded-xl">
                  <span className="block text-xl font-black text-brand-400">{vehicles.length}</span>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Fleet Vehicles</span>
                </div>
                <div className="p-3 bg-[#161F30] border border-[#23324C] rounded-xl">
                  <span className="block text-xl font-black text-brand-400">{customers.length}</span>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Shippers Setup</span>
                </div>
              </div>
        ...
        {/* --- BOTTOM ACTIONS --- */}
        {currentStep < 6 && (
          <div className="flex justify-between items-center pt-2 border-t border-[#23324C]/60">
            <button
              onClick={prevStep}
            ...
            <div className="flex items-center gap-3">
              {currentStep > 1 && currentStep < 6 && (
                <button
                  onClick={nextStep}
                  className="px-4 py-2.5 text-xs font-semibold text-slate-400 hover:text-slate-300 transition-colors"
                >
                  Skip Step
                </button>
              )}
