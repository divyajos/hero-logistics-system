...
export function Breadcrumbs({ crumbs = [] }) {
  ...
  return (
    <nav className="flex items-center space-x-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-500  font-mono select-none">
      {crumbs.map((crumb, idx) => {
        const isLast = idx === crumbs.length - 1;
        return (
          <React.Fragment key={idx}>
            {idx > 0 && <ChevronRight className="h-3 w-3 text-slate-600  flex-shrink-0" />}
            {isLast ? (
              <span className="text-slate-800  font-black truncate max-w-xs">{crumb}</span>
            ) : (
...
export function ConfirmationDialog({ 
  ...
  return (
    <div className="fixed inset-0 z-55 flex items-center justify-center p-4">
      {/* Backdrop */}
      <div 
        className="fixed inset-0 bg-slate-900/60 dark:bg-slate-50/80 backdrop-blur-xs transition-opacity" 
        onClick={onClose}
      ></div>

      {/* Modal Dialog Body */}
      <div className="bg-white dark:bg-slate-50 border border-slate-200 dark:border-slate-200 rounded-2xl w-full max-w-sm shadow-2xl relative z-10 animate-fade-in overflow-hidden p-6 text-center">
        {/* Warning Icon Banner */}
