  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-4 sm:pl-10">
        {/* Sliding Panel */}
        <div className="w-screen max-w-md bg-[#161F30] border-l border-[#23324C] shadow-2xl flex flex-col animate-slide-in-right relative">
          
          {/* Header */}
          <div className="p-5 border-b border-[#23324C]/60 flex items-center justify-between bg-[#0f1624]/40 h-16 shrink-0">
            <div>
              <div className="flex items-center space-x-2">
                <Bell className="h-5 w-5 text-brand-400" />
                <h2 className="text-sm sm:text-base font-extrabold text-white tracking-tight">Notification Center</h2>
              </div>
              <p className="text-[10px] text-slate-400 mt-0.5">{unreadCount} unread system logs</p>
            </div>
            <button onClick={onClose} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white transition-all cursor-pointer">
              <X className="h-4.5 w-4.5" />
            </button>
          </div>

          {/* Filter Bar */}
          <div className="p-4 border-b border-[#23324C]/60 space-y-3 bg-[#111827] shrink-0">
          ...
          {/* Notifications List */}
          <div className="flex-1 overflow-y-auto divide-y divide-[#23324C]/40 bg-[#0B0F19]/60">
            {filteredNotifications.length === 0 ? (
              ...
              filteredNotifications.map((n) => (
                <div key={n.id} className={`p-4 transition-colors flex items-start space-x-3 text-left border-l-2 ${getPriorityBorder(n.priority)} ${n.read ? 'opacity-70 bg-transparent' : 'bg-[#161F30]/40'}`}>
