  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-slate-900 capitalize">Dispatcher • {activeTab.replace('-', ' ')}</h2>
          <p className="text-xs text-slate-500">Match loads, track route geofences, and audit driver logs.</p>
        </div>
        ...
        <>
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="glass rounded-xl border border-slate-200/65 p-4 text-left">
                ...
                {showFiltersPanel && (
                  <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3 pt-4 border-t border-slate-200 mt-3 text-xs">
                    <div>
                      <label className="block text-slate-450 mb-1">Branch Terminal</label>
                      <select value={filterBranch} onChange={(e) => setFilterBranch(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none">
                    ...
                    <div>
                      ...
                      <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none">
                    ...
                    <div>
                      ...
                      <select value={filterDriver} onChange={(e) => setFilterDriver(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none">
                    ...
                    <div>
                      ...
                      <select value={filterCustomer} onChange={(e) => setFilterCustomer(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none">
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="e.g. Dallas" value={filterDestination} onChange={(e) => setFilterDestination(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none" />
                    </div>
                    <div>
                      <label className="block text-slate-450 mb-1">Required Date</label>
                      <input type="date" value={filterRequiredDate} onChange={(e) => setFilterRequiredDate(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none" />
                    </div>
                    <div>
                      <label className="block text-slate-450 mb-1">Niche Category</label>
                      <select value={filterNicheType} onChange={(e) => setFilterNicheType(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none">
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="e.g. TX-ROAD88" value={filterVehicleTrailer} onChange={(e) => setFilterVehicleTrailer(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-600 p-2 rounded-lg focus:outline-none" />
                    </div>
                    <div className="flex items-center pt-5">
                      <label className="flex items-center gap-2 cursor-pointer text-slate-600">
                        <input type="checkbox" checked={filterAvailableWorkersOnly} onChange={(e) => setFilterAvailableWorkersOnly(e.target.checked)} className="rounded border-slate-200 text-brand-500 focus:ring-brand-500 h-4.5 w-4.5" />
                    ...
                    <div className="flex items-end">
                      <button 
                        ...
                        className="w-full bg-white hover:bg-slate-700 py-2 rounded-lg text-slate-600 font-bold"
                      >
                        Reset Filters
                      </button>
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                ...
                <div className="lg:col-span-3 glass rounded-2xl p-4 border border-slate-200 text-left flex flex-col justify-between max-h-[500px] overflow-hidden">
                  <div>
                    <h3 className="text-sm font-extrabold text-slate-900 mb-2">Dispatcher Load List</h3>
                    <input 
                      type="text" 
                      placeholder="Quick search load/driver..." 
                      value={search} 
                      onChange={(e) => setSearch(e.target.value)} 
                      className="w-full mb-3 px-3 py-2 bg-slate-50 border border-slate-200 focus:border-brand-500 rounded-xl text-slate-700 text-xs focus:outline-none"
                    />
                  </div>
                        ...
                        <div 
                          ...
                          className={`p-3 rounded-xl border transition-all cursor-pointer text-left ${
                            (selectedOverviewLoad?.id === load.id)
                              ...
                              : 'bg-white/40 border-slate-200 hover:border-slate-200'
                          }`}
                        >
                          <div className="flex justify-between items-center text-[10px] mb-1">
                            <strong className="text-slate-900 font-mono">{load.loadId || `LD-${load.id}`}</strong>
                            <StatusBadge status={load.status} />
                          </div>
                          <h4 className="text-[11px] font-extrabold text-slate-700 truncate">{load.cargo}</h4>
                ...
                <div className="lg:col-span-5 glass rounded-2xl p-4 border border-slate-200 text-left flex flex-col justify-between max-h-[500px] overflow-hidden">
                  <div>
                    <h3 className="text-sm font-extrabold text-slate-900 mb-2 flex items-center justify-between">
                  ...
                  {selectedOverviewLoad ? (
                    <div className="flex-1 overflow-y-auto space-y-3.5 pr-1 scrollbar-thin">
                      <div className="p-3 bg-slate-50/60 border border-slate-200 rounded-xl text-xs space-y-2">
                        <div className="flex justify-between text-[10px] text-slate-500 font-bold uppercase">
                          <span>Route Summary</span>
                          <span>Niche: {selectedOverviewLoad.niche || 'General'}</span>
                        </div>
                        <p className="text-slate-900 font-extrabold">{selectedOverviewLoad.route}</p>
                        <p className="text-slate-500 text-[11px]">Vehicles assigned: <span className="text-brand-400 font-bold">{selectedOverviewLoad.vehicle || 'TX-ROAD88'}</span> | Trailer: <span className="text-brand-400 font-bold">{selectedOverviewLoad.trailer || 'TR-4022'}</span></p>
                      </div>
                      ...
                      <div className="space-y-3 pt-2">
                        ...
                        {(selectedOverviewLoad.stops || []).length === 0 ? (
                          <div className="p-4 border border-slate-200 rounded-xl text-slate-500 text-xs text-center">
                          ...
                          (selectedOverviewLoad.stops || []).map((stop, index) => (
                            <div key={index} className="flex gap-3 relative">
                              {index < selectedOverviewLoad.stops.length - 1 && (
                                <div className="absolute left-2.5 top-5 bottom-[-20px] w-0.5 bg-white" />
                              )}
                              ...
                              <div className="text-left text-xs">
                                <span className="px-1.5 py-0.5 rounded bg-white text-slate-500 text-[8px] font-bold uppercase tracking-wider">{stop.type}</span>
                                <strong className="text-slate-900 block mt-1">{stop.address}</strong>
                ...
                <div className="lg:col-span-4 glass rounded-2xl p-4 border border-slate-200 text-left flex flex-col justify-between max-h-[500px]">
                  <div>
                    <h3 className="text-sm font-extrabold text-slate-900 mb-1">Live GIS Route Map</h3>
                    <p className="text-[10px] text-slate-500">Fleet coordinates updated via telemetry feed.</p>
                  </div>

                  <div className="flex-grow bg-slate-50 rounded-xl border border-slate-200 relative overflow-hidden flex items-center justify-center my-3 min-h-[220px]">
              ...
              <div className="glass rounded-2xl p-4 border border-slate-200 text-left space-y-3">
                <h3 className="text-xs font-extrabold text-slate-900 uppercase tracking-wider">Driver Status Bar</h3>
                
                <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-thin">
                  {drivers.map((driver) => {
                    const matchedLoad = loads.find(l => l.driver === driver.name && l.status === 'In Transit');
                    return (
                      <div key={driver.id} className="min-w-[200px] p-3 bg-white/60 border border-slate-200 rounded-xl flex items-center justify-between gap-3 text-xs">
                        <div className="space-y-1">
                          <strong className="text-slate-900 block font-bold">{driver.name}</strong>
                        ...
                        <div className="flex flex-col gap-1 items-end">
                          ...
                          <div className="flex gap-1 mt-1">
                            <button onClick={() => triggerToast(`Dialing hotline to: ${driver.name}`)} className="p-1 bg-white hover:bg-slate-700 rounded text-slate-500" title="Call Driver">
                              <Phone className="h-3 w-3" />
                            </button>
                            <button onClick={() => triggerToast(`Open chat screen with: ${driver.name}`)} className="p-1 bg-white hover:bg-slate-700 rounded text-slate-500" title="Message Driver">
          ...
          {activeTab === 'create-load' && (
            <div className="flex flex-col space-y-6">
              {/* Header - Step 11 */}
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-lg">
                <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">Create Load Console</h3>
                  <p className="text-[10px] text-brand-400 font-bold tracking-widest mt-1">STEP 11: ACTIVATE / DISPATCH</p>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  <Button variant="outline" onClick={handleSaveDraft} className="flex-1 sm:flex-none border-slate-200 text-slate-600 hover:bg-white text-xs">
                    SAVE DRAFT
                  </Button>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 shadow-xl relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-slate-500"></div>
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider border-b border-slate-200 pb-3 mb-4 flex items-center gap-2">
                  ...
                  {/* Step 2: Add Stops */}
                  <div className="glass rounded-2xl p-6 border border-slate-200 shadow-xl relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Step 2: Add Stops</h4>
                          <p className="text-[10px] text-slate-500">Define pickup, layover, and delivery locations.</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={handleReorderStops} className="text-[10px] border-slate-200">
                          REORDER STOPS
                        </Button>
                        <Button variant="outline" size="sm" icon={Plus} onClick={handleAddStepperStop} className="text-[10px] border-slate-200 hover:border-blue-500/50 hover:text-blue-400 transition-colors">
                          ADD STOP
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {newStops.length === 0 ? (
                        ...
                        newStops.map((stop, idx) => (
                          <div key={stop.id} className="p-4 bg-slate-50/50 border border-slate-200 rounded-xl space-y-4 relative">
                            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                            ...
                            <p className="text-xs text-slate-900 font-bold">{stop.address}</p>
                  ...
                  <div className="glass rounded-2xl p-6 border border-slate-200 shadow-xl relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Step 3, 4, 5: Add Items & Links</h4>
                          <p className="text-[10px] text-slate-500">Declare items, Link to Stops, and Link Customer.</p>
                        </div>
                      </div>
                      <Button variant="outline" size="sm" icon={Plus} onClick={handleAddStepperItem} className="text-[10px] border-slate-200 hover:border-brand-500/50 hover:text-brand-400 transition-colors">
                        ADD ITEM
                      </Button>
                    </div>
                    ...
                    <div className="space-y-4">
                      {newItems.length === 0 ? (
                        <div className="p-4 bg-slate-50/50 border border-slate-200 rounded-xl space-y-4">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1 border-b border-slate-200 pb-4">
                            <div className="sm:col-span-1">
                              <span className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Step 5: Link Customer</span>
                              <TextInput value={itemCustLink} onChange={(e) => setItemCustLink(e.target.value)} placeholder="Select Customer Profile" />
                            </div>
                            <div className="sm:col-span-2 grid grid-cols-2 gap-4">
                              <div className="col-span-2"><span className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Step 4: Link Items to Stops</span></div>
                        ...
                        newItems.map((item, idx) => (
                          <div key={item.id} className="p-4 bg-slate-50/50 border border-slate-200 rounded-xl space-y-4 relative">
                            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className="glass rounded-2xl p-6 border border-slate-200 shadow-xl">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider border-b border-slate-200 pb-3 mb-4 flex items-center gap-2">
                      <Truck className="h-4 w-4 text-brand-400" /> Step 6, 7, 8: Assignments
                    </h4>
                    <div className="space-y-4">
                      <div>
                        <span className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Step 6: Assign Driver</span>
                      ...
                      <div>
                        <span className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Step 7: Assign Truck</span>
                      ...
                      <div>
                        <span className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Step 8: Assign Trailer</span>
                  ...
                  <div className="glass rounded-2xl p-6 border border-slate-200 shadow-xl">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider border-b border-slate-200 pb-3 mb-4 flex items-center gap-2">
                      <FileText className="h-4 w-4 text-slate-600" /> Step 9: Upload Documents
                    </h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div onClick={() => {setNewDocs({...newDocs, bol: true}); triggerToast('Manifest uploaded.');}} className={`border border-dashed ${newDocs.bol ? 'border-emerald-500 bg-emerald-500/10' : 'border-slate-200 bg-slate-50/50'} rounded-xl p-4 flex flex-col items-center justify-center text-center hover:border-brand-500/50 hover:bg-brand-500/5 transition-colors cursor-pointer min-h-[100px]`}>
                        <FileText className={`h-6 w-6 ${newDocs.bol ? 'text-emerald-400' : 'text-slate-500'} mb-2`} />
                        <span className="text-[10px] font-bold text-slate-600">Manifest / BOL</span>
                      </div>
                      <div onClick={() => triggerToast('Photos uploaded.')} className="border border-dashed border-slate-200 bg-slate-50/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:border-brand-500/50 hover:bg-brand-500/5 transition-colors cursor-pointer min-h-[100px]">
                  ...
                  <div className="glass rounded-2xl p-6 border border-slate-200 shadow-xl">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider border-b border-slate-200 pb-3 mb-4 flex items-center gap-2">
                      <MessageSquare className="h-4 w-4 text-slate-600" /> Internal Dispatch Notes
                    </h4>
                    <textarea 
                      value={newNotes}
                      onChange={(e) => setNewNotes(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-600 focus:outline-none focus:border-brand-500 resize-none min-h-[100px]"
                  ...
                  <div className="bg-brand-500/10 rounded-2xl p-6 border border-brand-500/30 shadow-xl">
                    ...
                    <p className="text-xs text-slate-600 mb-4">Review all parameters before activating. Load contains {newStops.length} stops and {newItems.length} items.</p>
            ...
            <div className="space-y-6">
              <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-5">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Review, confirm, match fields, or reject load manifests extracted automatically by AI model pipelines.</p>
                </div>
                ...
                <div className="space-y-6">
                  {aiQueue.loadInbox.filter(item => item.status === 'pending').length === 0 ? (
                    ...
                    aiQueue.loadInbox.filter(item => item.status === 'pending').map((item) => (
                      <div key={item.id} className="p-5 bg-white/80 border border-slate-200 rounded-2xl space-y-4">
                        <div className="flex justify-between items-center border-b border-slate-200/50 pb-3 flex-wrap gap-2">
                          <span className="text-[11px] text-slate-500 font-mono font-bold">Source manifest: <span className="text-slate-500 font-semibold">{item.source}</span></span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          <div className="p-3.5 bg-slate-900/60 border border-slate-200/45 rounded-xl space-y-2">
                            ...
                            <strong className="text-slate-900 text-xs block truncate">{item.data.customer || 'Vance Refrigeration'}</strong>
                            <button 
                              onClick={() => { if (window.confirm('Confirm Match Customer?')) triggerToast(`Matched with customer registry: ${item.data.customer}`) }}
                              className="w-full py-1 bg-white hover:bg-slate-750 text-slate-600 text-[10px] rounded-lg font-bold transition-all cursor-pointer"
                            >
                              Match Customer
                            </button>
                          </div>
                          ...
                          <div className="p-3.5 bg-slate-900/60 border border-slate-200/45 rounded-xl space-y-2">
                            ...
                            <strong className="text-slate-900 text-xs block truncate">{item.data.route || 'Chicago HQ ➔ St. Louis'}</strong>
                            <button 
                          ...
                          <div className="p-3.5 bg-slate-900/60 border border-slate-200/45 rounded-xl space-y-2">
                            ...
                            <strong className="text-slate-900 text-xs block">2 stops identified</strong>
                            <button 
                          ...
                          <div className="p-3.5 bg-slate-900/60 border border-slate-200/45 rounded-xl space-y-2">
                            ...
                            <strong className="text-slate-900 text-xs block truncate">{item.data.cargo || 'HVAC Units'}</strong>
                            <button 
                        ...
                        <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-200/35">
                          <button 
                            onClick={() => { if (window.confirm('Review AI Extract?')) triggerToast(`AI Raw JSON manifest metadata loaded for inspection.`) }}
                            className="px-3.5 py-2 bg-white hover:bg-slate-700 text-slate-700 text-xs rounded-xl font-bold transition-all cursor-pointer"
                          >
                            Review AI Extract
                          </button>
                          <button 
                          ...
                          <button 
                            ...
                            className="px-3.5 py-2 bg-slate-700 hover:bg-slate-650 text-slate-600 text-xs rounded-xl font-bold transition-all cursor-pointer"
                          >
                            Create Draft Load
                          </button>
                          <button 
                          ...
                          <button 
                            ...
                            className="px-3.5 py-2 bg-white hover:bg-slate-700 text-slate-450 text-xs rounded-xl font-bold transition-all cursor-pointer"
                          >
          ...
          {activeTab === 'active-loads' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                <div>
                  <h3 className="text-sm font-extrabold text-slate-900">Active Cargo Loads</h3>
              ...
              {filteredLoads.length === 0 ? (
                ...
                <>
                  <DataTable 
                    ...
                    columns={[
                      { key: 'route', label: 'Origin/Destination Path', render: (row) => <span className="font-extrabold text-slate-900">{row.route}</span> },
          ...
          {activeTab === 'planning-board' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <h3 className="text-sm font-extrabold text-slate-900">Active Cargo Timelines</h3>
              <div className="space-y-4">
                {loads.map(load => (
                  <div key={load.id} className="p-4 bg-white/40 border border-slate-200 rounded-xl space-y-3.5">
                    <div className="flex justify-between items-center text-xs">
                      <div>
                        <strong className="text-slate-900 block font-bold">{load.route}</strong>
                        <span className="text-[10px] text-slate-500 mt-0.5 block">Driver: {load.driver} • {load.weight}</span>
                      </div>
                      <StatusBadge status={load.status} />
                    </div>
          ...
          {activeTab === 'tracking' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch animate-fade-in">
              <div className="lg:col-span-8 glass rounded-2xl p-5 border border-slate-200 text-left flex flex-col justify-between h-[420px] lg:h-auto min-h-[380px] relative">
                <div>
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-sm font-extrabold text-slate-900 mb-1">Live GPS Fleet Positioning</h3>
                      <p className="text-[10px] text-slate-500">GIS overlay logs. Click coordinates to audit driver.</p>
                    </div>
                  </div>
                </div>
                
                <div className="flex-grow bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-center my-4 relative overflow-hidden min-h-[260px]">
                  <div className="absolute inset-0 bg-[radial-gradient(#23324c_1px,transparent_1px)] [background-size:16px_16px] opacity-40"></div>
                  <div className="absolute top-1/3 left-1/4 w-3 h-3 bg-brand-500 rounded-full border-2 border-[#0B0F19]" />
                  <div className="absolute top-1/2 right-1/4 w-3 h-3 bg-emerald-500 rounded-full border-2 border-[#0B0F19]" />
                </div>
              ...
              <div className="lg:col-span-4 glass rounded-2xl p-5 border border-slate-200 text-left flex flex-col justify-between h-[380px]">
                <div>
                  <h3 className="text-sm font-extrabold text-slate-900 mb-1">Dispatcher Communication Chat</h3>
                ...
                <div className="flex-1 overflow-y-auto space-y-3.5 my-4 pr-1 scrollbar-none flex flex-col-reverse">
                  {chats.map(chat => (
                    <div key={chat.id} className="p-3 bg-white/60 border border-slate-200 rounded-xl text-[11px]">
                ...
                <form onSubmit={handleSendMessage} className="flex gap-2 border-t border-slate-200 pt-3">
                  <input
                    ...
                    className="flex-grow px-3 py-2 bg-white border border-slate-200 focus:border-brand-500 rounded-xl text-slate-700 text-xs focus:outline-none"
                  />
          ...
          {activeTab === 'drivers' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-extrabold text-slate-900">Fleet Driver Availability</h3>
                <div className="flex gap-2">
              ...
              <DataTable 
                ...
                columns={[
                  { key: 'name', label: 'Driver Operator', render: (row) => <span className="font-extrabold text-slate-900">{row.name}</span> },
                  { key: 'status', label: 'Status', render: (row) => <StatusBadge status={row.status} /> },
                  { key: 'contact', label: 'Hotline', render: (row) => <span className="font-mono text-slate-500">{row.contact}</span> },
          ...
          {activeTab === 'vehicles-trailers' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-extrabold text-slate-900">Fleet Trailer Assets Registry</h3>
                <div className="flex gap-2">
              ...
              <DataTable 
                ...
                columns={[
                  { key: 'plate', label: 'Plate', render: (row) => <span className="font-mono text-slate-900">{row.plate}</span> },
          ...
          {activeTab === 'customers' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-extrabold text-slate-900">Shipper Accounts</h3>
              ...
              <DataTable
                ...
                columns={[
                  { key: 'scope', label: 'Scope', render: (row) => <span className="font-extrabold text-slate-900">{row.scope}</span> },
                  { key: 'type', label: 'Instruction Type', render: (row) => <span className="text-slate-500">{row.type}</span> },
          ...
          {activeTab === 'yard-warehouse' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4 animate-fade-in">
              <h3 className="text-sm font-extrabold text-slate-900">Storage Slots Grid</h3>
              <p className="text-xs text-slate-500">Yard capacity layout mapping trailer bays.</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {['Bay 1 (Reefer)', 'Bay 2 (Dry Van)', 'Bay 3 (Flatbed)'].map((bay, idx) => (
                  <div key={idx} className="p-4 bg-white/40 border border-slate-200 rounded-xl text-center space-y-2">
                    <span className="text-xs font-bold text-slate-700 block">{bay}</span>
          ...
          {activeTab === 'workforce-availability' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-extrabold text-slate-900">Worker Shift Roster Calendar</h3>
                <Button size="sm" variant="primary" onClick={() => triggerToast('Shift assigned successfully from Dispatch.')}>Assign Shift</Button>
              </div>
              <p className="text-xs text-slate-500">Roster calendar tracking driver logs.</p>
            </div>
          )}
          ...
          {activeTab === 'messages' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <h3 className="text-sm font-extrabold text-slate-900">Driver Hotline Chat Console</h3>
              <div className="flex gap-2">
          ...
          {activeTab === 'reports' && (
            <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
              <h3 className="text-sm font-extrabold text-slate-900">Dispatcher Delivery Performance reports</h3>
              <p className="text-xs text-slate-500">Total shipments dispatched per week metrics.</p>
            </div>
          )}
      ...
      <Drawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title="Active Route Inspector">
        {selectedLoadDetail && (
          <div className="space-y-6 text-left text-slate-500 text-xs sm:text-sm flex flex-col h-full">
            <div className="border-b border-slate-200 pb-4">
              <h4 className="text-base font-extrabold text-slate-900 mb-1">{selectedLoadDetail.route}</h4>
            ...
            <div className="flex-1 overflow-y-auto py-2">
              {drawerTab === 'details' && (
                <div className="space-y-4 animate-fade-in">
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div className="p-3 bg-white/10 border border-slate-800/40 rounded-xl">
                      <span className="text-[9px] text-slate-500 font-bold uppercase block">Driver</span>
                      <strong className="text-slate-900 text-xs block mt-1">{selectedLoadDetail.driver}</strong>
                      <span className="text-[9px] text-slate-500 block mt-0.5">{selectedLoadDetail.contact}</span>
                    </div>
                    <div className="p-3 bg-white/10 border border-slate-800/40 rounded-xl">
                      <span className="text-[9px] text-slate-500 font-bold uppercase block">Vehicle / Weight</span>
                      <strong className="text-slate-900 text-xs block mt-1">{selectedLoadDetail.vehicle || 'TX-ROAD88'}</strong>
                      <span className="text-[9px] text-slate-500 block mt-0.5">{selectedLoadDetail.weight}</span>
                    </div>
                  </div>

                  <div className="p-3 bg-[#0b0f19]/30 border border-slate-200 rounded-xl space-y-2">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide block">Operations Actions</span>
                  ...
                  <div className="mt-4 p-3 bg-[#0b0f19]/30 border border-slate-200 rounded-xl space-y-2">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide block">Advance Lifecycle Status</span>
                    <div className="flex gap-2">
                  ...
                  {['Draft', 'Planned', 'Assigned', 'Accepted', 'In Transit', 'Delivered', 'Closed'].map((step, idx, arr) => {
                    ...
                    return (
                      <div key={step} className="flex gap-3 relative">
                        {idx < arr.length - 1 && (
                          <div className={`absolute left-3 top-6 bottom-[-20px] w-0.5 bg-white`} />
                        )}
                        ...
                        <div className="text-left">
                          <h5 className={`text-xs font-bold ${isCurrent ? 'text-brand-500' : 'text-slate-500'}`}>{step}</h5>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  <div className="space-y-2">
                    {(selectedLoadDetail.documents || []).length === 0 ? (
                      ...
                      (selectedLoadDetail.documents || []).map((doc, dIdx) => (
                        <div key={dIdx} className="flex items-center justify-between p-3 bg-white/10 border border-slate-800/40 rounded-xl">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {(selectedLoadDetail.notes || []).length === 0 ? (
                      ...
                      (selectedLoadDetail.notes || []).map((note, nIdx) => (
                        <div key={nIdx} className="p-3 bg-white/10 border border-slate-200 rounded-xl text-left">
                          <p className="text-slate-500 font-semibold">{note}</p>
                        </div>
                      ))
                    )}
                  </div>
              ...
              {drawerTab === 'swap' && (
                <div className="space-y-4 animate-fade-in text-left text-xs">
                  ...
                  <div className="bg-white/40 border border-slate-200 rounded-xl p-4 space-y-3">
      ...
      <Modal isOpen={isAssignVehicleModalOpen} onClose={() => setIsAssignVehicleModalOpen(false)} title="Assign Existing Vehicle to Dispatch Load">
        <form onSubmit={handleAssignVehicleSubmit} className="space-y-4 p-1 text-left">
          ...
          {assignVehicleForm.vehicleId && (() => {
            ...
            if (!v) return null;
            return (
              <div className="grid grid-cols-2 gap-4 bg-white/20 p-3 rounded-lg border border-slate-200">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 block">Vehicle Plate</label>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 block">Vehicle Type</label>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 block">Current Status</label>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 block">Current Branch</label>
